...
frame_interval_minutes = 12 # frame interval of timecourse
...
df_force_recalc = False # whether to recalculate the dataframe in case anything upstream of this script changes
...
gen1_filter = (overall_df_with_rel['complete_cycle']==1)         \
...
# decay_start_time = 360 # realtime (min) where tet is shut off
# frame_interval_minutes = 12 # frame interval of timecourse
# frame_end = 61 # last frame to fit
# frame_start = 40 # first frame to fit; PICK SOMETHING ABOUT TWO HOURS AFTER SHUTOFF
# =============================================================================
...
# =============================================================================
# # DON'T TRUST EXPO FIT; it's a little too sensitive to where the fit starts
# # expo portion
...
sns.regplot(x=xvals,y=yvals, line_kws={'linewidth':5}) # linear fit
plt.plot(timepoints, totalsig/norm_factor, linewidth = 5) # actual data
# plt.errorbar(x=timepoints, y=totalsig/norm_factor, yerr=fluostderror, linewidth = 3) # actual 
# plt.plot(xvals,acdc_utils.expo_decay_fit(xvals,params[0],params[1],params[2]), linewidth = 10) # expo fit

plt.xlim([120,360])
plt.xlim([0,360])
plt.ylim([0,1.2])
plt.ylabel('Total fluo (all cells, normalized)')
plt.xlabel('Time relative to shutoff (min)')
plt.title(file_names[0] + ', slope = ' + str(m))
